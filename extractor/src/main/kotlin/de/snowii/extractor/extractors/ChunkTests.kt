...
class ChunkTest(val name: String, val seed: Long, val x: Int, val z: Int) : Extractor.Extractor {

    companion object {
        public fun readTests(): List<Extractor.Extractor> {
            // The CWD is the ./run folder
            val reader = FileReader("../chunk_tests.json")
            val gson = Gson()
            val tests = gson.fromJson(reader, Array<ChunkTest>::class.java)
            return tests.toList()
        }
    }
    ...
    private fun createFluidLevelSampler(
            settings: ChunkGeneratorSettings
    ): AquiferSampler.FluidLevelSampler {
        ...
        return AquiferSampler.FluidLevelSampler { _, y, _ ->
            if (y < Math.min(-54, i)) fluidLevel else fluidLevel2
        }
    }
                            ...
                            for (s in (0 ..< l).reversed()) {
                                ...
                                for (w in 0 ..< k) {
                                    ...
                                    for (z in 0 ..< k) {
                                        ...
                                        val index =
                                                this.get_index(config, y, t - config.minimumY(), ab)
    ...
    override fun extract(server: MinecraftServer): JsonElement {
        ...
        val test_sampler =
                ChunkNoiseSampler(
                        16 / shape.horizontalCellBlockCount(),
                        config,
                        chunk_pos.startX,
                        chunk_pos.startZ,
                        shape,
                        object : DensityFunctionTypes.Beardifying {
        ...
        data?.forEach { state -> topLevelJson.add(state) }
