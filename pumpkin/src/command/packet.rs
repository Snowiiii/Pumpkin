...
fn write_dispatcher_to_bytebuf<'a>(
    dispatcher: &&CommandDispatcher<'a>,
    bytebuf: &'a mut ByteBuffer,
) {
    ...
    bytebuf.put_list(&proto_nodes, |bytebuf, node: &ProtoNode| {
    ...
    fn from_node(node: &Node<'a>, offset: i32) -> Option<Self> {
        ...
        match node.node_type {
            ...
            NodeType::Literal { string, .. } => {
                Some(ProtoNode::new_literal(children, string, true))
            }
