...
fn generate_block_state_collision_shapes() {
    let json_data: Vec<Vec<u16>> =
        serde_json::from_str(include_str!("../assets/block_state_collision_shapes.json"))
            .expect("Could not parse json file.");

    let shapes: Vec<_> = json_data
        .into_iter()
        .map(|block_state_shape_idxs| {
            quote::quote! {
    ...
    save_to_file(
        "block_state_collision_shapes.rs",
        quote::quote! {
...
fn generate_shapes() {
    let json_data: Vec<JsonShape> =
        serde_json::from_str(include_str!("../assets/block_shapes.json"))
            .expect("Could not parse json file.");
...
fn generate_block_state_properties() {
    let json_data: Vec<Vec<&str>> =
        serde_json::from_str(include_str!("../assets/block_state_properties.json"))
            .expect("Could not parse json file.");

    let properties: Vec<_> = json_data
        .into_iter()
        .map(|block_state_property_values| {
            quote::quote! {
...
fn generate_block_states() {
    let json_data: Vec<JsonBlockState> =
        serde_json::from_str(include_str!("../assets/block_states.json"))
            .expect("Could not parse json file.");

    let states: Vec<_> = json_data
        .into_iter()
        .map(|s| {
...
fn save_to_file(name: &str, tokens: proc_macro2::TokenStream) {
    ...
    let mut file = std::fs::File::create(out_path.join(name)).expect("Failed to create file");

    let parsed = syn::parse2(tokens).expect("Failed to parse TokenStream");
